...
contract ContinueOnRevertHandler is Test {
    ...
    //@note we create aHandler, where we define all functions (from the engine) that should be called by the fuzzer with potential pre-conditions
    // Functions: depositCollateral ; redeemCollateral ; burnDsc ; mintDSC ; liquidate
    // FUNCTOINS TO INTERACT WITH

    ///////////////
    // DSCEngine //
    ///////////////
    // we only deposit collateral => no DSC is minted (only collateral tokens are minted)
    function mintAndDepositCollateral(uint256 collateralSeed, uint256 amountCollateral) public {
        //@note limit input vars using bound
    ...
    function burnDsc(uint256 amountDsc) public {
        //@note of course, we need to limit this to the amout the caller currently holds => cant burn more DSC than what is held
        amountDsc = bound(amountDsc, 0, dsc.balanceOf(msg.sender));
        dsc.burn(amountDsc);
    }

    //no DSC is minted via the engine (after depositing collat) => this is directly minted from DSC token
    ...
    /// Helper Functions
    //@note function that returns pseudo-randomly one of 2 collateral tokens
    ...
    //@note is not called by fuzzer, because the function is external
